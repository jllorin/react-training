...
function App() {
  ...
  const handleClickCalculate = (e: any) => {
    ...
    alert(`${fullName} is adding ${myInitial.addWith} to the number ${myInitial.noOfTimes} times. New value is ${number}`);        
  }

  return <div>
      <div style={{padding: "10px", background: "yellow"}}>
        {`${fullName} is adding ${myInitial.addWith} times ${myInitial.noOfTimes} to the number ${number}`}
      </div>
      <div>
